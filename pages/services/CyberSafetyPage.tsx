...
const CyberSafetyPage: React.FC<CyberSafetyPageProps> = ({ onBack }) => {
    ...
    const handleReportToCyberCell = async (source: 'manual' | 'intercepted') => {
        ...
        if (source === 'manual' && analysisResult) {
             reportData = { senderId: "Unknown/Hidden", contentSample: inputText, evidenceImage: screenshot || undefined, category: analysisResult.category, threatLevel: analysisResult.level, user: { name: user.name, contact: user.phone, location: user.location.coords } };
        } else if (source === 'intercepted' && interceptedMessage) {
            reportData = { senderId: interceptedMessage.sender, contentSample: interceptedMessage.content, category: interceptedMessage.detectedCategory, threatLevel: interceptedMessage.threatLevel, user: { name: user.name, contact: user.phone, location: user.location.coords } };
        }
        if (reportData) {
            await mockApi.createCyberReport(reportData);
